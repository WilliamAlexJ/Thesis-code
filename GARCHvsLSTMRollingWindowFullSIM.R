...
dm_test_tick <- function(loss1, loss2) {
  ...
  list(
    DM_stat  = DM_stat,
    p_value  = p_value,
    mean_diff = d_bar   # >0=EGARCH worse than LSTM
  )
}


#EGARCH(1,1)-t ROLLING VAR
...
rolling_lstm_var <- function(log_returns,
  ...
  for (t in window_size:(n - 1)) {
    if (is.na(last_trained_t) || (t - last_trained_t) >= retrain_every) {
      ...
      last_trained_t <- t
      cat("    [", cfg_name, "] alpha", alpha, ": retrained at t =", t, "\n")
    }
...
#MAIN LOOP: EGARCH VS MULTI-LSTM
...
for (alpha in alpha_vec) {
  cat("\n======================================\n")
  cat("Running experiments for alpha =", alpha, "\n")
  cat("======================================\n")
  
  #EGARCH rolling VaR
  cat("  - Computing EGARCH VaR...\n")
  ...
  for (cfg in lstm_configs) {
    ...
    cat("\n  - Computing LSTM VaR for config:", cfg_name, "\n")
...
plot_VaR <- function(dates, ret, VaR_eg, VaR_lstm, alpha, cfg_name) {
  
  plot(dates, ret, type="l", col="black", 
       main=paste("VaR Forecasts (alpha =", alpha, ") for", cfg_name),
       ylab="Return", xlab="Date")
  
  lines(dates, VaR_eg, col="red", lwd=2)
  lines(dates, VaR_lstm, col="blue", lwd=2)
  
  legend("bottomleft",
         legend=c("Returns", "EGARCH VaR", paste(cfg_name, "VaR")),
         col=c("black", "red", "blue"),
         lwd=2, bty="n")
}

plot_VaR(
  dates[start_oos:n_total],
  ret_oos,
  VaR_eg_oos,
  VaR_lstm_oos,
  alpha = 0.01,
  cfg_name = "LSTM_lb30_e20"
)




plot_exceedances <- function(dates, ret, VaR, alpha, model_name) {
  exceed <- ret < VaR

  plot(dates, ret, type="l", col="gray30",
       main=paste("Exceedances for", model_name, "(alpha =", alpha, ")"),
       ylab="Return", xlab="Date")

  lines(dates, VaR, col="red", lwd=2)
  points(dates[exceed], ret[exceed], col="blue", pch=19)
}
...
roll_freq <- function(exceed, window=250) {
  zoo::rollapply(exceed, window, mean, align="right", fill=NA)
}

plot(dates[start_oos:n_total],
     roll_freq(exceed_eg, 250),
     type="l", col="red", lwd=2,
     main="Rolling Exceedance Frequency (250-day)",
     ylab="Exceedance Rate", xlab="Date")
...
plot(dates[start_oos:n_total], loss_eg_use, type="l", col="red",
     main="Tick Loss (EGARCH)", ylab="Loss", xlab="Date")

plot(dates[start_oos:n_total], loss_lstm_common, type="l", col="blue",
     main=paste("Tick Loss (", cfg_name, ")", sep=""),
     ylab="Loss", xlab="Date")
...
plot(VaR_eg_use, ret_eg_use,
     pch=20, col=rgb(1,0,0,0.5),
     main="Return vs EGARCH VaR", xlab="VaR", ylab="Return")
abline(0, 1)
...
ggplot(dm_summary, aes(x=config, y=DM_stat, fill=factor(alpha))) +
  geom_bar(stat="identity", position="dodge") +
  geom_hline(yintercept=0, lwd=1) +
  ggtitle("Diebold–Mariano Statistics vs EGARCH") +
  theme_bw()
...
hist(log_returns, breaks=100, col="gray80", freq=FALSE)
lines(density(log_returns), col="blue", lwd=2)
...
image(weights, col=heat.colors(100),
      main="LSTM Weight Matrix Heatmap")
